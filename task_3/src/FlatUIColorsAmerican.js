...
function FlatUIColorsAmerican() {
  const [flatUIColorsAmericanPalette, setFlatUIColorsAmericanPalette] = useState(null);

  useEffect(() => {
    fetch('palette.json')
      .then(response => response.json())
      .then(data => {
        const foundPalette = data.find(palette => palette.paletteName === 'Flat UI Colors American');
        setFlatUIColorsAmericanPalette(foundPalette);
      })
  ...
  if (!flatUIColorsAmericanPalette) {
  ...
  const groupedColors = chunkArray(flatUIColorsAmericanPalette.colors, 4);

  return (
    <div className="palette-page">
      <h3>{flatUIColorsAmericanPalette.paletteName}</h3>
...
export default FlatUIColorsAmerican;
