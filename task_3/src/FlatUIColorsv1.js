...
function FlatUIColorsv1() {
  const [flatUIColorsPalette, setFlatUIColorsPalette] = useState(null);

  useEffect(() => {
    fetch('palette.json')
      .then(response => response.json())
      .then(data => {
        const foundPalette = data.find(palette => palette.paletteName === 'Flat UI Colors v1');
        setFlatUIColorsPalette(foundPalette);
      })
