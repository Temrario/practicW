...
function MaterialUIColors() {
  const [materialUIColorsPalette, setMaterialUIColorsPalette] = useState(null);

  useEffect(() => {
    fetch('palette.json')
      .then(response => response.json())
      .then(data => {
        const foundPalette = data.find(palette => palette.paletteName === 'Material UI Colors');
        setMaterialUIColorsPalette(foundPalette);
      })
  ...
  return (
    <div className="palette-page">
      <h3>{materialUIColorsPalette.paletteName}</h3>
