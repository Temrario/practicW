...
function FlatUIColorsDutch() {
  const [flatUIColorsDutchPalette, setFlatUIColorsDutchPalette] = useState(null);

  useEffect(() => {
    fetch('palette.json')
      .then(response => response.json())
      .then(data => {
        const foundPalette = data.find(palette => palette.paletteName === 'Flat UI Colors Dutch');
        setFlatUIColorsDutchPalette(foundPalette);
      })
  ...
  const groupedColors = chunkArray(flatUIColorsDutchPalette.colors, 4);

  return (
    <div className="palette-page">
      <h3>{flatUIColorsDutchPalette.paletteName}</h3>
...
export default FlatUIColorsDutch;
