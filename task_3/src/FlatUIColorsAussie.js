...
function FlatUIColorsAussie() {
  const [flatUIColorsAussiePalette, setFlatUIColorsAussiePalette] = useState(null);

  useEffect(() => {
    fetch('palette.json')
      .then(response => response.json())
      .then(data => {
        const foundPalette = data.find(palette => palette.paletteName === 'Flat UI Colors Aussie');
        setFlatUIColorsAussiePalette(foundPalette);
      })
  ...
  const groupedColors = chunkArray(flatUIColorsAussiePalette.colors, 4);

  return (
    <div className="palette-page">
      <h3>{flatUIColorsAussiePalette.paletteName}</h3>
...
export default FlatUIColorsAussie;
