...
function FlatUIColorsSpanish() {
  const [flatUIColorsSpanishPalette, setFlatUIColorsSpanishPalette] = useState(null);

  useEffect(() => {
    fetch('palette.json')
      .then(response => response.json())
      .then(data => {
        const foundPalette = data.find(palette => palette.paletteName === 'Flat UI Colors Spanish');
        setFlatUIColorsSpanishPalette(foundPalette);
      })
  ...
  const groupedColors = chunkArray(flatUIColorsSpanishPalette.colors, 4);

  return (
    <div className="palette-page">
      <h3>{flatUIColorsSpanishPalette.paletteName}</h3>
...
export default FlatUIColorsSpanish;
